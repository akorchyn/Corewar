...
static void	lst_print(t_list *list)
{
	while (list)
	{
		...
		ft_printf("\t\t\t\t\t\t%lu\n", GET_INSTRUCTION(list)->lexem.size);
		ft_printf("\t\t\t\t\t\t\t\t\t\t%lu\n", GET_INSTRUCTION(list)->size_from_start);
		list = list->next;
	}
}
...
static void	write_output(char *file_name)
{
	...
	read_command_argument(g_instructions);
	calculate_sizes(g_instructions);
	put_global_sizes(g_instructions);
	parse_labels(g_instructions);
