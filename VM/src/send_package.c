...
void	send_(int32_t sock, void *package, int32_t size, int32_t mode)
{
	int32_t		sended;
...
** Next		info about 4 players, some may be empty
** 								4 * (id + name), id - int8_t, name - 128 int8_t
...
void	send_init_package(t_corewar *corewar)
{
	...
	package_iter = 0;
	size = 1 + 4 * (sizeof(uint8_t) + PROG_NAME_LENGTH) + 3 * 4;
	...
	while (++i < 4)
	{
		if (i < corewar->players)
		{
			...
			ft_memcpy(init_package + package_iter + 1, g_header[i]->prog_name,
					PROG_NAME_LENGTH);
		}
	...
	send_(corewar->sock, (void *)init_package, size, 0);
	free(init_package);
}
